...
\section{Methodology}
\noindent
We followed a research through design methodology, in which the interface was first derived from theory and then evaluated for legibility in a controlled online study. The design phase drew on Science and Technology Studies to decide how the two biosignals should and should not be represented, and the evaluation phase tested whether the resulting visual language could be identified and interpreted as intended. This section describes the theoretical grounding of the design, the icon system it produced, and the study through which that system was assessed.

\subsection{Design Approach and Theoretical Grounding}
The interface was produced through research creation, an approach in which building an artifact is itself a mode of inquiry \cite{amos_kyriaki_2015}, extended by speculative design, which uses a working artifact to probe how a technology might be arranged otherwise \cite{wakkary_things_2021}. Science and Technology Studies supplied the rationale, and each visual decision followed from one of three commitments. Because measurement helps constitute the phenomenon it appears to record \cite{nelson_meeting_1996}, the system reads heart rate and variability relative to each player's own baseline rather than against an absolute clinical scale, treating the display as a produced reading rather than a disclosed fact. Because the categories an interface installs carry consequences and harden into infrastructure \cite{bowker_sorting_2008, winner_artifacts_1980}, the design admits only a small set of relative visual states and never a named feeling. Because any reading of a body is situated and partial \cite{haraway_situated_1988}, and meaning forms in place rather than arriving fixed inside a symbol \cite{suchman_plans_1987, dourish_where_2004}, the cues are deliberately open and take their sense from the surrounding match. Prior work reading skin conductance between friends shows that such openness is a resource rather than a defect, since one bodily rise carried many meanings depending on the exchange it accompanied \cite{howell_biosignals_2016}.

\subsection{The Icon System}
The design placed the two signals on two visual channels so each could be read on its own. Heart rate, an index of momentary cardiovascular activation, drove pulse speed, glow, and halo scale. Heart rate variability, a marker of autonomic regulation estimated from the spacing between beats \cite{thayer_model_2000, porges_polyvagal_2007, shaffer_overview_2017}, drove the coherence of a surrounding ring, ranging from a smooth continuous halo to a broken and jittering one. Each channel carried four states defined relative to baseline, yielding sixteen combinations, each animated for five to seven seconds. The mapping was kept compositional rather than assigning an unrelated picture to every pairing, so that a viewer who had learned the two channels could read a combination never seen before. No state was shown with an emotion word.
...
The evaluation used a within participants design in which each participant encountered all sixteen states, which removed between person variation from the central comparisons and produced dense repeated observations. The study assessed whether the visual language was legible and interpreted as intended. It did not test whether an icon altered a viewer's own physiology, and it did not require live capture, since the stimuli presented simulated cardiovascular states through the same animation logic the deployed system would use. Streaming a wearable's heart rate into a game engine is already established \cite{liang_harnessing_2024}, so the open question was interpretation rather than instrumentation.

\subsection{Participants}
Thirty six adults took part. Most were between 18 and 34, played regularly, and varied in their familiarity with wearable devices, and most completed the study on a desktop browser. Consent was obtained before any stimulus appeared, no identifying information was recorded, and the biosignal states shown were synthetic, which kept the procedure low in risk.

\subsection{Apparatus and Materials}
The prototype ran in a standard web browser and rendered each state with the animation logic described above, presenting one avatar within a simulated team status panel. Presenting the sixteen states through this prototype, rather than inside a full game, isolated the reading of the display from the demands of live play and let every participant see an identical stimulus set.

\subsection{Procedure}
The session moved through two phases. In an open phase, with every label hidden, participants viewed a state, described in free text what it conveyed, chose the closest option from a set of plain descriptions, and rated how activated and how stable the figure appeared. This followed the open logic of comprehension testing, where an unprompted answer reveals what a symbol communicates on its own. In a structured phase, participants saw all sixteen states in randomized order and, for each, identified the heart rate state and the variability state from four alternatives, rated clarity, urgency, perceived stability, usefulness for coordination, and any intention to offer support, and completed the affective rating. A closing questionnaire recorded workload, confidence, visual comfort, willingness to use the display in cooperative play, and open remarks.

\subsection{Measures}
Three established instruments anchored the measurement. Comprehension was assessed in the manner of graphical symbol testing, which checks whether a symbol conveys its intended meaning without accompanying text \cite{iso_9186_2014}, and was scored as identification accuracy for the two states. Affective interpretation used the Self Assessment Manikin, a nonverbal pictorial scale for valence, arousal, and dominance that avoids emotion vocabulary \cite{bradley_measuring_1994}, a fitting match for a display that avoids it as well. Effort used the NASA Task Load Index, a multidimensional workload scale spanning mental demand, effort, and frustration \cite{hart_development_1988}, to confirm that reading the halo did not draw on attention the game needs. Additional single item ratings captured the social reading of each state, including urgency, perceived stability, and the intention to offer support.

\subsection{Analysis}
Identification accuracy was compared against the 25 percent expected from guessing among four alternatives. Whether interpretation ordered itself with the underlying states was tested with rank correlation, checking that urgency and arousal climbed with activation and that perceived stability fell, while the intention to help rose, as ring coherence declined. Workload was summarized across the Task Load Index dimensions. Free text answers were analyzed thematically through an established reflexive coding procedure \cite{braun_using_2006}, to characterize how participants made sense of the states and to gather their concerns about physiological visibility.
